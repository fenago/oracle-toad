...
set feedback off
DECLARE
 CURSOR C1 IS SELECT * FROM oe.orders;
 CURSOR C2 IS SELECT * FROM oe.order_items;
 TYPE list IS TABLE OF VARCHAR2(30);
 parameters list; 
 base_string VARCHAR2(80) := 'select * from oe.ORDERS where order_id = ::1';
 statement_str VARCHAR2(105); 
 return_str C1%ROWTYPE;
BEGIN 
  SELECT TO_CHAR(order_id) BULK COLLECT INTO parameters FROM oe.ORDERS;
